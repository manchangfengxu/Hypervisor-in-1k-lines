...
use spin::Mutex;
use crate::linux_loader::{PLIC_ADDR, PLIC_END};
...
extern "C" fn handle_trap(vcpu: *mut VCpu) -> ! {
    ...
    match scause {
        1 => {
            ...
            // This usually means the Guest jumped to a high virtual address 
            // but the G-stage page table doesn't have a mapping for it.
            panic!("Guest Instruction Access Fault at sepc={:#x}, stval={:#x}", sepc, stval);
        }
        10 /* environment call from VS-mode */ => {
...
trap_handler:
    ...
    # Restore a0 from sscratch, and save it into VCpu.
    csrr t0, sscratch
    sd t0, {a0_offset}(a0)
